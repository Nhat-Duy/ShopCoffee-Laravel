...
<html>
    ...
    <aside class="max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full -translate-x-full flex-wrap items-center justify-between overflow-y-auto rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0 xl:bg-transparent">
      <div class="h-19.5">
        ...
        <a class="block px-8 py-6 m-0 text-sm whitespace-nowrap text-slate-700" href="javascript:;" target="_blank">
          <img src="{{asset('public/backend/img/logo-ct.png')}}" class="inline h-full max-w-full transition-all duration-200 ease-nav-brand max-h-8" alt="main_logo" />
      ...
      <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full">
        <ul class="flex flex-col pl-0 mb-0">
          <li class="mt-0.5 w-full">
            <a class="py-2.7 shadow-soft-xl text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap rounded-lg bg-white px-4 font-semibold text-slate-700 transition-colors" href="{{URL::to('/dashboard')}}">
          ...
          <li class="mt-0.5 w-full">
            <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{URL::to('/danhmuc')}}">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Danh mục sản phẩm</span>
            </a>
          </li>
          

          <li class="mt-0.5 w-full">
            <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{URL::to('/sanpham')}}">
          ...
          <li class="mt-0.5 w-full">
            <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{URL::to('/quanlybinhluan')}}">
              <div class="shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center fill-current stroke-0 text-center xl:p-2.5">
                <svg width="12px" height="12px" viewBox="0 0 43 36" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                  <title>credit-card</title>
                  <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
              ...
              <span class="ml-1 duration-300 opacity-100 pointer-events-none ease-soft">Quản lý bình luận</span>
            </a>
          </li>

          <li class="mt-0.5 w-full">
            <a class="py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors" href="{{URL::to('/quanlydonhang')}}">
              <div class="shadow-soft-2xl mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-white bg-center stroke-0 text-center xl:p-2.5">
                <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <!-- plugin for charts  -->
  <script src="{{asset('public/backend/js/plugins/chartjs.min.js')}}" async></script>
  ...
  <script type="text/javascript">
    $('.binhluan_duyet_btn').click(function(){
        ...
        var id_bl = $(this).data('comment_id');
        var id_sp_bl = $(this).attr('id');
        if(comment_status == 0){
        ...
        $.ajax({
              url : '{{url('/duyet_binhluan')}}',
              method: 'POST',
              headers:{
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
              },
              data: {comment_status:comment_status, id_bl:id_bl, id_sp_bl:id_sp_bl},
              success:function(data){
                location.reload();
                $('#notify_comment').html('<p>'+ alert +'</p>')
              }
        });
    });
    $('.traloibinhluan').click(function(){
        var id_bl = $(this).data('comment_id');
        var binhluan = $('.traloi_'+id_bl).val();
        var id_sp_bl = $(this).data('product_id');

        // alert(binhluan);
        // alert(id_bl);
        // alert(id_sp_bl);
        $.ajax({
              url : '{{url('/traloi_binhluan')}}',
              method: 'POST',
              headers:{
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
              },
              data: {binhluan:binhluan, id_bl:id_bl, id_sp_bl:id_sp_bl},
              success:function(data){
                location.reload();
                $('.traloi_'+id_bl).val('');
                $('#notify_comment').html('<p>Trả lời bình luận thàng công</p>')
              }
        });
    });
  </script>
